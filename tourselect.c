...
void selection (population *old_pop, population *new_pop)
{
    ...
    {
        parent1 = mono_tournament (&old_pop->ind[a1[i]], &old_pop->ind[a1[i+1]]);
        parent2 = mono_tournament (&old_pop->ind[a1[i+2]], &old_pop->ind[a1[i+3]]);
        crossover (parent1, parent2, &new_pop->ind[i], &new_pop->ind[i+1]);
        /*
        printf("\nParent1");
        print_ind(   &old_pop->ind[a1[i]]   , 1  );
        print_ind(   &old_pop->ind[a1[i+1]]  , 1);
        print_ind(   parent1  , 1);

        printf("\nParent2");
        print_ind(   &old_pop->ind[a1[i+2]]   , 1  );
        print_ind(   &old_pop->ind[a1[i+3]]  , 1);
        print_ind(   parent2  , 1);
        */

        parent1 = mono_tournament (&old_pop->ind[a2[i]], &old_pop->ind[a2[i+1]]);
        parent2 = mono_tournament (&old_pop->ind[a2[i+2]], &old_pop->ind[a2[i+3]]);
        ...
        */
        ...
        /*
        print_ind(&new_pop->ind[i], 1);
        print_ind(&new_pop->ind[i+1], 1);
        print_ind(&new_pop->ind[i+2], 1);
        print_ind(&new_pop->ind[i+3], 1);
		*/
...
{
    int flag;
    flag = check_optimality (ind1, ind2);
    if (flag==1)
    {
        return (ind1);
    }
