...
void mono_fill_nondominated_sort (population *mixed_pop, population *new_pop)
{
    ...
    do
    {
        ...
        do
        {
            ...
            {
            	//compara individuos
            	//busca dominancia, si el ind1 domina, el ind2 se elimina de elite y regresa al pool
                end = 0;
                flag = check_optimality (&(mixed_pop->ind[temp1->index]), &(mixed_pop->ind[temp2->index]));
                if (flag == 1)
                {
                    insert (pool, temp2->index);
                    temp2 = del (temp2);
            ...
            //si el ind1 domina se agrega a la lista elite
            if (flag == 0 || flag == 1)
            {
                insert (elite, temp1->index);
                front_size++;
        ...
        if ( (archieve_size+front_size) <= popsize)
        {
            ...
            while (temp2 != NULL);
            //Asigna crowding_distance a la frontera actual
            //assign_crowding_distance_indices (new_pop, j, i-1);
            rank+=1;
        }
        else
        {
        	//ordena el arreglo de la mixed_pop basado en su crowding_distance
            //crowding_fill (mixed_pop, new_pop, i, front_size, elite);
            archieve_size = popsize;
            for (j=i; j<popsize; j++)
            {
